...
void dae::TextObject::Update(const float deltaTime)
{
	...
	if (mNeedsUpdate)
	{
		...
		auto texture = SDL_CreateTextureFromSurface(m_ServiceLocator.GetRenderer()->GetSDLRenderer(), surf);
		if (texture == nullptr) 
		{
...
void dae::TextObject::Render() const
{
	if (mTexture != nullptr)
	{
		...
		m_ServiceLocator.GetRenderer()->RenderTexture(*mTexture, pos.x, pos.y);
	}
}
